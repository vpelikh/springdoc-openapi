...
/**
 * Verifies that [KotlinRequiredPropertyCustomizer] correctly processes
 * Kotlin data classes via [io.swagger.v3.core.util.KotlinDetector.isKotlinClass].
 */
class KotlinRequiredPropertyCustomizerKotlinTest {

    private val mapperProvider: ObjectMapperProvider = mock(ObjectMapperProvider::class.java)
    private val customizer = KotlinRequiredPropertyCustomizer(mapperProvider)
    ...
    @Test
    fun `should mark non-nullable field as required`() {
        ...
        assertThat(result!!.required)
            .isNotNull
            .contains("nonNullField")
            .doesNotContain("nullableField", "fieldWithDefault")
    }

    @Test
    fun `should mark non-nullable deprecated field as required`() {
        val type = AnnotatedType().`type`(KotlinTestFixture::class.java)
        ...
        assertThat(result).isSameAs(unresolved)
        assertThat(result!!.required).contains("deprecatedField", "nonNullField")
    }
