...
/**
 * Verifies that [KotlinDeprecatedPropertyCustomizer] correctly processes
 * Kotlin data classes via [io.swagger.v3.core.util.KotlinDetector.isKotlinClass].
 */
class KotlinDeprecatedPropertyCustomizerKotlinTest {

    private val mapperProvider: ObjectMapperProvider = mock(ObjectMapperProvider::class.java)
    private val customizer = KotlinDeprecatedPropertyCustomizer(mapperProvider)
    ...
    @Test
    fun `should mark deprecated field as deprecated`() {
        ...
        val unresolved = Schema<Any>().apply {
            ...
            addProperty("deprecatedField", io.swagger.v3.oas.models.media.StringSchema())
        }
        ...
        assertThat(result!!.properties!!["deprecatedField"]?.getDeprecated()).isTrue()
    }

    @Test
    fun `should not mark non-deprecated field as deprecated`() {
        val type = AnnotatedType().`type`(KotlinTestFixture::class.java)
        ...
        assertThat(result).isSameAs(unresolved)
        assertThat(result!!.properties!!["nonNullField"]?.getDeprecated()).isNull()
    }
